...
@Component({
  selector: 'app-with-issues',
  templateUrl: './with-issues.component.html',
  styleUrls: ['./with-issues.component.scss']
})
export class WithIssuesComponent implements OnInit {
...
inputcompany;
  ...
  viewresearch(view): void {
    const dialogRef = this.dialog.open(ViewResearchDetailComponent, {
      width: '70%',data:{data:view}, disableClose: false });
  ...
  createTable() {
        this.http.get(this.global.api + 'api.php?action=spResearch_List&company=null&status=2',
         this.global.option)
            .map(response => response.json())
            .subscribe(res => {
            this.tableArr= res;
