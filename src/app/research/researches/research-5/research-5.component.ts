...
@Component({
  selector: 'app-research-5',
  templateUrl: './research-5.component.html',
  styleUrls: ['./research-5.component.scss']
})
export class Research5Component implements OnInit {
  ...
  createTable() {
        this.http.get(this.global.api + 'api.php?action=spResearch_List&company=' + this.global.useraccess.companyid+"&status=6",
         this.global.option)
            .map(response => response.json())
            .subscribe(res => {
            this.tableArr= res;
