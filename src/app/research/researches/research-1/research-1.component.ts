...
@Component({
  selector: 'app-research-1',
  templateUrl: './research-1.component.html',
  styleUrls: ['./research-1.component.scss']
})
export class Research1Component implements OnInit {
  ...
  createTable() {
        this.http.get(this.global.api + 'api.php?action=spResearch_List&company=' + this.global.useraccess.companyid+"&status=1",
         this.global.option)
            .map(response => response.json())
            .subscribe(res => {
            this.tableArr= res;
             ...
             let urlSearchParams2 = new URLSearchParams();
              ...
              var header = new Headers();
               ...
               this.http.post(this.global.api + 'api.php?action=spResearchResearchStatus_Insert',body2,option2)
                    .map(response => response.text())
                    .subscribe(res => {
                      this.createTable();this.global.swalSuccess("Changed status to Draft!")
                    });
          }
        }
      })
