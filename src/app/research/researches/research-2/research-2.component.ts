...
@Component({
  selector: 'app-research-2',
  templateUrl: './research-2.component.html',
  styleUrls: ['./research-2.component.scss']
})
export class Research2Component implements OnInit {
  ...
  createTable() {
        this.http.get(this.global.api + 'api.php?action=spResearch_List&company=' + this.global.useraccess.companyid+"&status=3",
         this.global.option)
            .map(response => response.json())
            .subscribe(res => {
            this.tableArr= res;
